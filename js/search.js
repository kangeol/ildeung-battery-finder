(function () {
  ...
  function populateBrands() {
    resetSelect(brandSelect, "브랜드 선택", false);
    activeBrands().forEach((brand) => {
      brandSelect.append(option(brand.id, brand.nameKr));
    });
    status.textContent = dbSource === "localStorage"
      ? "저장된 관리자 수정 DB를 사용 중입니다."
      : "기본 JSON DB를 사용 중입니다.";
  }

  function populateModels() {
    resetSelect(modelSelect, brandSelect.value ? "차종 선택" : "브랜드를 먼저 선택", !brandSelect.value);
  ...
  function populateYears() {
    ...
    resetSelect(yearSelect, years.length ? "연식 선택" : "등록된 연식 없음", !years.length);
    resetSelect(fuelSelect, "연식을 먼저 선택");
    resultList.innerHTML = "";
    emptyState.hidden = false;
    ...
    status.textContent = years.length ? "연식을 선택해 주세요." : "등록된 차량 상세가 없습니다.";
  }
  ...
  function populateFuels() {
    ...
    resetSelect(fuelSelect, fuels.length ? "연료 선택" : "등록된 연료 없음", !fuels.length);
    resultList.innerHTML = "";
    emptyState.hidden = false;

    fuels.forEach((fuel) => fuelSelect.append(option(fuel)));
    status.textContent = fuels.length ? "연료를 선택하고 결과를 확인해 주세요." : "등록된 연료 정보가 없습니다.";
  }
  ...
  function renderResults(variant) {
    ...
    if (!matches.length) {
      ...
      emptyState.innerHTML = `
        <strong>등록된 배터리 매칭이 없습니다.</strong>
        <p>전화상담으로 정확한 배터리를 확인해 주세요.</p>
        <a class="btn primary" href="tel:1644-9141">전화상담 1644-9141</a>
      `;
    ...
    resultList.innerHTML = matches.map(({ match, part }) => `
      <article class="result-card">
        <div class="result-top">
          <div>
            <span class="badge">priority ${match.priority}</span>
            <h3>${vehicleName}</h3>
          </div>
        ...
        <div class="spec-grid">
          ...
          <div class="spec-box">
            <span class="spec-label">배터리 브랜드</span>
            <span class="spec-value">${part.brand}</span>
          </div>
          <div class="spec-box">
            <span class="spec-label">관리 제조사</span>
            <span class="spec-value">${part.manufacturer || "-"}</span>
          </div>
        </div>
        <div class="price-row">
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (!brandSelect.value) {
      status.textContent = "브랜드를 선택해 주세요.";
      brandSelect.focus();
      return;
    }
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      ...
      console.error(error);
      status.textContent = "DB를 불러오지 못했습니다. GitHub Pages 환경에서 다시 확인해 주세요.";
    }
  });
})();
